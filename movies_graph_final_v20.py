...
if __name__ == "__main__":
    # Set wide layout mode in Streamlit
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title='Director-Genre-Movie Knowledge Graph with Influence Detection',
            titlefont_size=16,
            showlegend=False,
            hovermode='closest',
    ...
    # Display the Streamlit app interface
    st.title('Movies Knowledge Graph')
    st.subheader('What is this, and why should you care?')
    st.write("[Knowledge Graphs](https://www.turing.ac.uk/research/interest-groups/knowledge-graphs) are a useful way to visualize data in a network structure. You should care because this unique analysis method can be used in so many ways - from facilitating new drug discoveries to recommending your next Netflix movie. The graph below runs on the [Kaggle Movies Dataset](https://www.kaggle.com/datasets/utkarshx27/movies-dataset/data) - it's built using [NetworkX](https://networkx.org/documentation/stable/auto_examples/graph/index.html) & the [Louvain method](https://en.wikipedia.org/wiki/Louvain_method) to detect & measure influential nodes (movies, directors, genres). Nodes are color-coded & sized based on their influence. Hover over nodes for more info.")

    # Display the Plotly figure using Streamlit
    st.plotly_chart(fig, use_container_width=True)

    # Display the filtered dataset underneath the graph
    st.subheader("Filtered Dataset")

    # Filter the original dataset ('data') based on nodes in 'G_filtered'
    filtered_data = data[data['title'].isin(G_filtered.nodes())]

    st.dataframe(filtered_data.set_index('title'), width=900)  # Display the filtered dataset DataFrame without the index column
