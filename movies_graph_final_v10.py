...
if __name__ == "__main__":
    ...
    st.title('Movie-Director-Genre Knowledge Graph with Influence Detection')
    st.write("This visualization uses NetworkX and the Louvain method to detect & measure influential nodes (movies, directors, genres) within the graph. Nodes are color-coded and sized based on their influence, with larger and more intense colors indicating higher influence within the network. Hover over nodes to see detailed information including community, centrality measures, and roles.")
    ...
    st.dataframe(data.set_index('title'), width=900)  # Display the filtered dataset DataFrame without the index column
